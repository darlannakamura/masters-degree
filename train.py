...
def train_mlp(file: str, check: bool):
    ...
    mlp.fit(epochs=get_epochs(check), batch_size=128, shuffle=True, extract_validation_dataset=True)

    mlp.save_loss_plot(os.path.join(config['output_path'], f'mlp_loss.png'))

def train_cnn(file: str, check: bool):
...
def train_autoencoder(file: str, check: bool):
...
def train_cgan(file: str, check: bool):
...
def train_dncnn(file: str, check: bool):
...
def train(filename: str, check: bool):
    for func in [train_mlp, train_cnn, train_autoencoder, train_cgan, train_dncnn]:
        p = Process(target=func, args=(filename, check, ))
        p.start()
        p.join()
