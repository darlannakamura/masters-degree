...
from denoising.methods.neural_network.cgan_denoiser.data_processing import padding_power_2
...
def make_generator_model_small(train_size=28):
    f = config.base_number_of_filters
    k = config.kernel_size
    s = config.strides
    sz = train_size
...
def make_discriminator_model(train_size=28):
    f = config.base_number_of_filters
    k = config.kernel_size
    s = config.strides
    sz = train_size
